...
main_task::main_task()
{
  xTaskCreate( &main_task_code_c_wrapper, "main_task", configMINIMAL_STACK_SIZE, NULL, MAIN_TASK_PRIORITY, this->main_task_handle ); /* Create main_task */
}
...
#ifdef DEBUG
  if( main_task_queue_handle == NULL ) std::printf( "[!] Main task Queue creation error!\r\n" );
  else std::printf( "[+] Main task Queue created!\r\n" );
...
#ifdef DEBUG
  if( main_task_semaphore_handle == NULL ) std::printf( "[!] Main task Semaphore creation error!\r\n" );
  else std::printf( "[+] Main task Semaphore created!\r\n" );  
#endif /*DEBUG*/
...
std::vector< std::string > main_task::parse_command( std::string * command )
{
  ...
  for( std::string str; command_stream >> str; ) args_list.push_back( str ); 
...
void main_task::exec_command( std::vector< std::string > & args_list )
{
  ...
  if( args_list[ 0 ] == "#erase" ){
	...
	for( unsigned int i = 0; i < args_list.size(); i++ ) if( i != 0 ) ( i < args_list.size() - 1 ) ? std::printf( "%s, ", args_list[ i ].data() ) : std::printf( "%s\r\n", args_list[ i ].data() );
	pc_usart_driver::send_format_string( "\r\n[+] Command \"%s\" parsed! Args : ", args_list[ 0 ].c_str() );
	for( unsigned int i = 0; i < args_list.size(); i++ ) if( i != 0 ) ( i < args_list.size() - 1 ) ? pc_usart_driver::send_format_string( "%s, ", args_list[ i ].data() ) : pc_usart_driver::send_format_string( "%s\r\n", args_list[ i ].data() );
#endif/* DEBUG */
	...
	if(( args_list[ 1 ] == "pages" ) && ( args_list[ 3 ] == "to" )){
	  ...
	  * stop_page_stream >> stop_page;
	  ...
	  for( unsigned int i = start_page; i <= stop_page; i++ ){
		...
		spi_flash_driver::spi_flash_read_buffer(( uint8_t * ) &* page_buffer->c_str(), SPI_FLASH_ORIGIN_ADDR + i * W25X_SPI_PAGESIZE, W25X_SPI_PAGESIZE );
		pc_usart_driver::send_format_string( "[+] Flash data at page #%u, addr %#x : %s\r\n", i, SPI_FLASH_ORIGIN_ADDR + i * W25X_SPI_PAGESIZE, page_buffer->c_str() );
		delete page_buffer;
	  }	  
	  ...
	  * page_num_stream >> page_num;
	  ...
	  std::string * page_buffer = new std::string;
	  page_buffer->resize( W25X_SPI_PAGESIZE );
	  spi_flash_driver::spi_flash_read_buffer(( uint8_t * ) &* page_buffer->c_str(), SPI_FLASH_ORIGIN_ADDR + page_num * W25X_SPI_PAGESIZE, W25X_SPI_PAGESIZE );
	  pc_usart_driver::send_format_string( "[+] Flash data at page #%u, addr %#x : %s\r\n", page_num, SPI_FLASH_ORIGIN_ADDR + page_num * W25X_SPI_PAGESIZE, page_buffer->c_str() );
	  delete page_buffer;
	  pc_usart_driver::send_format_string( "[+] OK!\r\n" );
	  ...
	  * test_num_stream >> test_num;
	  ...
	  spi_flash_driver::spi_flash_read_buffer(( uint8_t * ) &* page_buffer->c_str(), SPI_FLASH_FILE_ADDR + test_num * W25X_SPI_PAGESIZE, RELAYS_COUNT );
	  pc_usart_driver::send_format_string( "[+] Test #%u data at addr %#x : %s\r\n", test_num, SPI_FLASH_FILE_ADDR + test_num * W25X_SPI_PAGESIZE, page_buffer->c_str() );
	  delete page_buffer;
	  pc_usart_driver::send_format_string( "[+] OK!\r\n" );
	  ...
	  * stop_test_stream >> stop_test;
	  ...
	  for( unsigned int i = start_test; i <= stop_test; i++ ){
		...
		std::string * page_buffer = new std::string;
		page_buffer->resize( RELAYS_COUNT );
		spi_flash_driver::spi_flash_read_buffer(( uint8_t * ) &* page_buffer->c_str(), SPI_FLASH_FILE_ADDR + i * W25X_SPI_PAGESIZE, RELAYS_COUNT );
		pc_usart_driver::send_format_string( "[+] Test #%u data at addr %#x : %s\r\n", i, SPI_FLASH_FILE_ADDR + i * W25X_SPI_PAGESIZE, page_buffer->c_str() );
		delete page_buffer;
	...
	if( args_list[ 1 ] == "page" ){
	  ...
	  * page_num_stream >> page_num;
	  ...
	  std::string * page_buffer = new std::string;
	  page_buffer->resize( W25X_SPI_PAGESIZE );
	  spi_flash_driver::spi_flash_write_page(( uint8_t * ) &* args_list[ 3 ].c_str(), SPI_FLASH_ORIGIN_ADDR + page_num * W25X_SPI_PAGESIZE, args_list[ 3 ].length() );
	  spi_flash_driver::spi_flash_read_buffer(( uint8_t * ) &* page_buffer->c_str(), SPI_FLASH_ORIGIN_ADDR + page_num * W25X_SPI_PAGESIZE, W25X_SPI_PAGESIZE );
	  ...
	  * test_num_stream >> test_num;
	  ...
	  if( * buffer != 0xff ){
		...
		pc_usart_driver::send_format_string( "[!] Erasing sector at address : %#x\r\n", SPI_FLASH_FILE_ADDR + W25X_SPI_SECTOR_SIZE * ( test_num * W25X_SPI_PAGESIZE ) / W25X_SPI_SECTOR_SIZE );
#endif /* DEBUG */
		
		spi_flash_driver::spi_flash_erase_sector( SPI_FLASH_FILE_ADDR + W25X_SPI_SECTOR_SIZE * ( test_num * W25X_SPI_PAGESIZE ) / W25X_SPI_SECTOR_SIZE );

	  } else {

#ifdef DEBUG
		std::printf( "[+] Sector at address %#x already erased.\r\n", SPI_FLASH_FILE_ADDR + W25X_SPI_SECTOR_SIZE * ( test_num * W25X_SPI_PAGESIZE ) / W25X_SPI_SECTOR_SIZE );
		pc_usart_driver::send_format_string( "[+] Sector at address %#x already erased.\r\n", SPI_FLASH_FILE_ADDR + W25X_SPI_SECTOR_SIZE * ( test_num * W25X_SPI_PAGESIZE ) / W25X_SPI_SECTOR_SIZE );
#endif /* DEBUG */
	  ...
	  std::string * page_buffer = new std::string;
	  page_buffer->resize( RELAYS_COUNT );
	  spi_flash_driver::spi_flash_write_page(( uint8_t * ) &* args_list[ 3 ].c_str(), SPI_FLASH_FILE_ADDR + test_num * W25X_SPI_PAGESIZE, args_list[ 3 ].length() );
	  spi_flash_driver::spi_flash_read_buffer(( uint8_t * ) &* page_buffer->c_str(), SPI_FLASH_FILE_ADDR + test_num * W25X_SPI_PAGESIZE, RELAYS_COUNT );
#ifdef DEBUG
	...
	if( args_list[ 1 ] == "load_test" ){
	  ...
	  * test_num_stream >> test_num;
	  ...
	  (( page_buffer->c_str()[ 0 ] == '0' ) || ( page_buffer->c_str()[ 0 ] == '1' )) ? fpga_stream = new std::bitset< RELAYS_COUNT >( page_buffer->c_str() ) : fpga_stream = new std::bitset< RELAYS_COUNT >( 0b0 );
...
void main_task::pc_usart_callback( USART_TypeDef * USARTx )
{
  if( USARTx == PC_USART_PORT ){
	...
	if( USART_GetITStatus( USARTx, PC_USART_IT_VECTOR_TYPE )){
	  ...
	  if( usart_irq_main_task_woken == pdTRUE ) taskYIELD();
...
bool main_task::flash_test()
{
  ...
  spi_flash_driver::spi_flash_write_page(( uint8_t * ) &* write_buffer->c_str(), SPI_FLASH_ORIGIN_ADDR, std::strlen( "FLASH TEST!" ));
  ...
  spi_flash_driver::spi_flash_read_buffer(( uint8_t * ) &* read_buffer->c_str(), SPI_FLASH_ORIGIN_ADDR, std::strlen( "FLASH TEST!" ));
...
bool main_task::global_test()
{
  ...
  std::printf( "[!] SPI chip jedec device ID : %#x\r\n", spi_flash_driver::spi_flash_read_jedec_id() );
  std::printf( "[!] SPI chip unique device ID : %#x\r\n", spi_flash_driver::spi_flash_read_unique_id() );
  std::printf( "[!] Erase sectors of flash memory from %#x to %#x ...\r\n", SPI_FLASH_ORIGIN_ADDR, SPI_FLASH_ORIGIN_ADDR + W25X_SPI_SECTOR_SIZE * ( RELAYS_COUNT / ( W25X_SPI_SECTOR_SIZE / W25X_SPI_PAGESIZE )));
  pc_usart_driver::send_format_string( "[!] SPI chip manufacturer device ID : %#x\r\n", spi_flash_driver::spi_flash_read_id() );
  pc_usart_driver::send_format_string( "[!] SPI chip jedec device ID : %#x\r\n", spi_flash_driver::spi_flash_read_jedec_id() );
  pc_usart_driver::send_format_string( "[!] SPI chip unique device ID : %#x\r\n", spi_flash_driver::spi_flash_read_unique_id() );
  pc_usart_driver::send_format_string( "[!] Erase sectors of flash memory from %#x to %#x ...\r\n", SPI_FLASH_ORIGIN_ADDR, SPI_FLASH_ORIGIN_ADDR + W25X_SPI_SECTOR_SIZE * ( RELAYS_COUNT / ( W25X_SPI_SECTOR_SIZE / W25X_SPI_PAGESIZE )));
#endif /* DEBUG */

  for( unsigned int i = 0; i < RELAYS_COUNT / ( W25X_SPI_SECTOR_SIZE / W25X_SPI_PAGESIZE ); i++ ) spi_flash_driver::spi_flash_erase_sector( SPI_FLASH_ORIGIN_ADDR + W25X_SPI_SECTOR_SIZE * i );
  
  for( unsigned int i = 0; i < RELAYS_COUNT; i++ ){
	
#ifdef DEBUG
	std::printf( "[!] Write fpga bitstream to flash memory ( Page #%u, addr = %#x ) : %s\r\n", i + 1, SPI_FLASH_ORIGIN_ADDR + i * W25X_SPI_PAGESIZE, fpga_write_stream->to_string().c_str() );
	pc_usart_driver::send_format_string( "[!] Write fpga bitstream to flash memory ( Page #%u, addr = %#x ) : %s\r\n", i + 1, SPI_FLASH_ORIGIN_ADDR + i * W25X_SPI_PAGESIZE, fpga_write_stream->to_string().c_str() );
#endif /* DEBUG */
	
	spi_flash_driver::spi_flash_write_page(( uint8_t * ) fpga_write_stream->to_string().c_str(), SPI_FLASH_ORIGIN_ADDR + i * W25X_SPI_PAGESIZE, fpga_write_stream->to_string().length() );
	std::string * read_buffer = new std::string;
	read_buffer->resize(std::strlen( fpga_write_stream->to_string().c_str() ));
	spi_flash_driver::spi_flash_read_buffer(( uint8_t * ) &* read_buffer->c_str(), SPI_FLASH_ORIGIN_ADDR + i * W25X_SPI_PAGESIZE, fpga_write_stream->to_string().length() );
	std::bitset< RELAYS_COUNT > * fpga_read_stream = new std::bitset< RELAYS_COUNT >( * read_buffer );
#ifdef DEBUG
	std::printf( "[!] Reading data from flash ( Page #%u, addr = %#x ) : %s\r\n", i + 1, SPI_FLASH_ORIGIN_ADDR + i * W25X_SPI_PAGESIZE, fpga_read_stream->to_string().c_str() );
	...
	if( std::strcmp( fpga_write_stream->to_string().c_str(), read_buffer->c_str() ) != 0 ) return false;
...
{
  ( reinterpret_cast< main_task * >( pvParams ))->main_task_code( pvParams );
}

void USART2_IRQHandler( main_task * task )
{
  task->pc_usart_callback( PC_USART_PORT );
}
