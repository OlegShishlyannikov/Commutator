...
bootloader::bootloader() {
  ...
  std::printf("\r\nName : %s\r\nHardware version : %s\r\nSoftware version : %s\r\nAuthor : %s\r\nAuthor contacts : "
              "%s\r\nCompany : %s\r\nCompany address : %s\r\n",
              this->hardware_name_ptr->c_str(), this->hardware_version_ptr->c_str(),
              this->software_version_ptr->c_str(), this->author_name_ptr->c_str(), this->author_mail_ptr->c_str(),
              this->company_name_ptr->c_str(), this->company_address_ptr->c_str());
  std::printf("\r\nADCCLK : %u Hz\r\nSYSCLK : %u Hz\r\nHCLK : %u Hz\r\nPCLK1 : %u Hz\r\nPCLK2 : %u Hz\r\n\r\n",
              this->adcclk_freq, this->sysclk_freq, this->hclk_freq, this->pclk1_freq, this->pclk2_freq);
}

const char *bootloader::get_hardware_name() { return this->hardware_name_ptr->c_str(); }

const char *bootloader::get_author_name() { return this->author_name_ptr->c_str(); }

const char *bootloader::get_author_mail() { return this->author_mail_ptr->c_str(); }

const char *bootloader::get_company_name() { return this->company_name_ptr->c_str(); }

const char *bootloader::get_company_address() { return this->company_address_ptr->c_str(); }

const char *bootloader::get_hardware_version() { return this->hardware_version_ptr->c_str(); }

const char *bootloader::get_software_version() { return this->software_version_ptr->c_str(); }
...
void bootloader::init_hardware() {
  ...
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_USART2 | RCC_APB1Periph_SPI2,
                         ENABLE); /* Enable clocking on PC_USART and FPGA SPI */
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO | RCC_APB2Periph_USART1 | RCC_APB2Periph_GPIOA | RCC_APB2Periph_GPIOB |
                             RCC_APB2Periph_GPIOC | RCC_APB2Periph_GPIOD | RCC_APB2Periph_SPI1,
                         ENABLE); /* Enable clocking on all GPIO ports and SPI_FLASH spi port */
